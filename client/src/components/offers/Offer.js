...
const Offer = () => {
  ...
  useEffect(() => {
    if (offer) {
      const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${offer.lng},${offer.lat}.json?access_token=${process.env.REACT_APP_MAP_TOKEN}`;
      fetch(url)
  ...
  const handleClose = () => {
    dispatch({ type: 'UPDATE_OFFER', payload: null });
  };
  return (
    <Dialog
      fullScreen
      ...
      <Container sx={{ pt: 5 }}>
        ...
        <Stack sx={{ p: 3 }} spacing={2}>
          ...
          >
            <Box>
              ...
              <Typography component="span">
                {offer?.price === 0 ? 'Negtiable' : '$' + offer?.price}
              </Typography>
            </Box>
            <Box
              sx={{
          ...
          <Stack>
            ...
            <Typography component="span">{offer?.description}</Typography>
          </Stack>
        </Stack>
      </Container>
    </Dialog>
